...
| Auto-launch | auto-launch.ts | Done -- HKLM Run key (machine-wide) with HKCU fallback (v0.3.7) |
...
| CRX2 to CRX3 format fix | v0.3.3 | Chrome 145 on WorkSpaces rejected the CRX2 binary with CRX_HEADER_INVALID. pack-extension.js rewritten to use Chrome's built-in `--pack-extension` command instead of manual CRX2 binary assembly. Chrome's packer always outputs CRX3 format. Script finds chrome.exe in Program Files (both x64 and x86) or PATH, runs `--pack-extension` + `--pack-extension-key`, moves output to build/valerie-url-bridge.crx. Same PEM key, same extension ID (lpdlfbkigloncemklhgcclimjfbglfkk). Build machine must have Chrome installed. |
| URL debug logging | v0.3.4 | Added diagnostic logging to url-bridge.ts and window-tracker.ts for tracing the extension-to-bridge-to-tracker chain. URLBridge logs incoming POSTs, cache updates, and getLastUrl() results with age. Window tracker logs URL lookup results and attachment status for Chrome samples. |
| CRX registry to enterprise force-install | v0.3.5 | Chrome 145 blocks sideloaded extensions via CRX external extension registry ("can't verify" error). Switched to Chrome enterprise policy force-install via HKLM\SOFTWARE\Policies\Google\Chrome\ExtensionInstallForcelist. NSIS installer writes update.xml manifest (gupdate format) to C:\ProgramData\ValerieAgent\ pointing to local CRX file. Old CRX registry keys under HKLM\SOFTWARE\Google\Chrome\Extensions\ cleaned up on both install and uninstall. Reboot persistence verified on WorkSpace. All files on C: drive, all registry in HKLM -- safe for golden image capture. |
| tzOffset in time-entries call | v0.3.6 | Agent sends tzOffset query parameter to GET /api/tracker/time-entries so server calculates 'today' in VA's local timezone. |
| HKLM auto-launch | v0.3.7 | Auto-launch changed from HKCU (per-user) to HKLM (machine-wide). NSIS installer writes HKLM Run key at install time. Runtime auto-launch.ts tries HKLM first, falls back to HKCU if not elevated. Cleans up old HKCU key on successful HKLM write. Fixes golden image issue where new VA profiles had no auto-launch entry. |
...
| Item | Detail |
|------|--------|
| Installer | Valerie Agent Setup 0.3.7.exe |
| Size | ~81 MB |
...
3. **Download the installer** from GitHub Releases (Valerie Agent Setup 0.3.7.exe)
...
- URLs not visible in dashboard: Agent sends url in sync payload as of v0.3.0. Va-platform needs to add url String? to WindowSample Prisma model -- sync Zod schema silently strips it.

**Va-platform integration requirements (handoff):**
1. Add url String? to WindowSample Prisma model + sync route Zod schema
...
**URL tracking chain verified (v0.3.5):**
- Full chain: Chrome extension -> localhost HTTP bridge (127.0.0.1:19876) -> window-tracker getLastUrl() -> sync payload
- Extension installs via Chrome enterprise force-install policy (ExtensionInstallForcelist) -- survives reboot on WorkSpace
- CRX registry approach (v0.3.1-v0.3.4) abandoned -- Chrome 145 blocks sideloaded extensions with "can't verify" error
