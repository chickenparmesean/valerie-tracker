// Defensive console wrapper — prevents EPIPE crash when piped to PowerShell during shutdown
const originalConsoleLog = console.log;
const originalConsoleError = console.error;
console.log = (...args: any[]) => {
  try { originalConsoleLog(...args); } catch (e) { /* EPIPE — shutting down */ }
};
console.error = (...args: any[]) => {
  try { originalConsoleError(...args); } catch (e) { /* EPIPE — shutting down */ }
};
...
if (!gotTheLock) {
  ...
  app.on('before-quit', () => {
    ...
    try { stopSyncEngine(); } catch (e) {}
    try { stopAutoUpdater(); } catch (e) {}
