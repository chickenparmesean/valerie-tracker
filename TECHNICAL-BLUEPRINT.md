...
All recommended packages were used and work correctly on AWS WorkSpaces. Testing was conducted on a real AWS WorkSpace. Current stable version is v0.3.7. Versions v0.2.1-v0.2.5 added debug logging, screenshot privacy gating, single instance lock, graceful shutdown, Chrome page title tracking, project refresh button, and today total display. v0.2.6 fixed screenshot metadata URLs and removed screenshot notification. v0.2.7 fixed stale timer resume after reboot and added auto-stop on prolonged unanswered idle. v0.2.8 added close warning dialog on window close when timer is running and wired note input end-to-end to sync payload. v0.3.0 added Chrome extension URL tracking via a Manifest V3 extension that POSTs the active tab URL to a localhost HTTP bridge (127.0.0.1:19876) in the Electron agent; the agent attaches the URL to Chrome window samples in the sync payload. v0.3.1 added CORS headers on the URL bridge (required for extension fetch), fixed app display name in Task Manager, and switched to CRX packaging with a persistent RSA signing key for reproducible builds. v0.3.2 wired pack-extension.js into the build pipeline as a prebuild step. v0.3.3 rewrote pack-extension.js to produce CRX3 format via chrome.exe --pack-extension (Chrome 145 rejects CRX2 with CRX_HEADER_INVALID). v0.3.4 added debug logging to url-bridge.ts and window-tracker.ts for the extension-to-bridge-to-tracker chain. v0.3.5 switched Chrome extension install from CRX external extension registry to enterprise force-install policy (ExtensionInstallForcelist) -- Chrome 145 blocks sideloaded extensions with "can't verify" error even with valid CRX3; enterprise policy bypasses this. Agent now syncs to va-platform at staging.hirevalerie.com.
...
**URL tracking (v0.3.0-v0.3.5):** A Manifest V3 Chrome extension (`agent/chrome-extension/`) tracks the active tab URL via `chrome.tabs.onActivated` and `chrome.tabs.onUpdated`, and POSTs it to a localhost HTTP bridge (`agent/src/main/url-bridge.ts`) at `127.0.0.1:19876`. The window tracker reads the cached URL via `getLastUrl()` and attaches it to Chrome window samples as the `url` field. Internal Chrome pages (`chrome://`, `about:`, `devtools://`, `chrome-extension://`) are filtered to null by the extension. The URL bridge has a 30-second staleness check -- if the extension stops reporting, `getLastUrl()` returns null. Gated behind the `trackUrls` config flag. Uses zero new npm dependencies (Node.js built-in `http` module for the bridge, plain JavaScript for the extension). Debug logging added in v0.3.4 for the full extension-to-bridge-to-tracker chain.

**CRX3 build process (v0.3.3):** `agent/build/pack-extension.js` shells out to `chrome.exe --pack-extension` to produce a CRX3-format binary. Chrome 145 rejects the older CRX2 format with `CRX_HEADER_INVALID`. The script finds chrome.exe in Program Files (both x64 and x86) or PATH, runs `--pack-extension` with `--pack-extension-key` pointing to `agent/build/extension.pem`, and moves the output to `build/valerie-url-bridge.crx`. Build machine must have Chrome installed. The prebuild step runs automatically before electron-builder in both `build:agent` and `publish:agent` scripts (wired in v0.3.2).
...
The deployment workflow: launch a standard WorkSpace, install the Electron app, the NSIS installer writes a **machine-wide Registry Run key** (`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`) for auto-launch at login -- this persists across all user profiles in golden images (v0.3.7), create an AWS WorkSpace Image (takes ~45 minutes), then create a Custom Bundle from that image. All future WorkSpaces launched from this bundle will have the tracker pre-installed and auto-starting. For updates, the auto-updater handles it—no need to rebuild the golden image for each release.
