...
5. ~~Write WorkSpace provisioning script that drops config.json with the API key~~ DONE -- va-platform's `sync-activity` cron calls `deployTrackerConfig()` which uses AWS SSM RunCommand to write config.json (containing only `apiKey` and `apiBaseUrl`) to `C:\ProgramData\ValerieAgent\config.json` when a VA is hired and their WorkSpace becomes AVAILABLE
...
- On startup, read config file:
  ...
  ```
  **Only `apiKey` and `apiBaseUrl` are required.** The server resolves `orgId` and `userId` from the API key. All other settings are fetched from the server via `/api/tracker/config`.
  ```json
  {
...
### Why this matters
In production, va-platform provisions this config file automatically via AWS SSM RunCommand. When a VA is hired and their WorkSpace becomes AVAILABLE, the va-platform `sync-activity` cron calls `deployTrackerConfig()` which writes config.json (containing only `apiKey` and `apiBaseUrl`) to `C:\ProgramData\ValerieAgent\config.json`. The VA opens their WorkSpace and the tracker is already running, authenticated, showing their tasks. **No login screen, no API key entry, no onboarding friction.** The server resolves `orgId` and `userId` from the API key -- no need to include them in config.json.

For testing, you manually create the config.json file on the test WorkSpace with just `apiKey` and `apiBaseUrl`.
...
- URLs captured from Chrome, sent through localhost HTTP bridge, attached to WindowSample sync payloads
- Sync to staging.hirevalerie.com succeeds for all record types
- Stale URL expiry works (null after 30s)
- Screenshots capture, compress to WebP, queue for upload
- Activity polling, idle detection, timer start/stop all correct

**Config.json only requires `apiKey` and `apiBaseUrl`** -- the server resolves `orgId` and `userId` from the API key. In production, va-platform provisions config.json automatically via AWS SSM RunCommand (`deployTrackerConfig()`).

**URL tracking observation:** Full URLs including query parameters are captured (e.g., LinkedIn authwall URLs with tracking tokens). Future enhancement: URL truncation or query param stripping for privacy/storage efficiency.

**Note:** va-platform currently drops the `url` field on ingest -- WindowSample model missing the `url` column, sync Zod schema silently strips it. This is a va-platform task, not an agent bug.
...
| url field | -- | Not yet accepted in sync Zod schema or stored in WindowSample table. Agent sends `url` (string or null) on window samples as of v0.3.0. Va-platform needs to add `url String?` to WindowSample Prisma model -- sync Zod schema silently strips it. |

### Previously broken, now resolved on va-platform

| Item | Status |
|------|--------|
| /api/tracker/time-entries GET | RESOLVED -- endpoint built |
| pageTitle field | RESOLVED -- column added to WindowSample |
| Screenshots presign 400 | RESOLVED -- field mismatch fixed |
| trackUrls config flag | RESOLVED -- returned from GET /api/tracker/config |
...
```json
{
  "timeEntries": [{
    ...
    "taskId": "string | null",
    "note": "string | null (now populated via note input, was always null before v0.2.8)"
  }],
  "activitySnapshots": [{
    "idempotencyKey": "uuid",
    "timestamp": "ISO 8601",
