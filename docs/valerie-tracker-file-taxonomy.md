...
| `ipc.ts` | [UI] [INTEGRATION] | IPC handler registration. Maps renderer API calls to main process functions: auth, timer, projects, config, idle, tasks, app version, today total. |

## agent/src/preload/ -- Preload Bridge

| File | Tags | Description |
|------|------|-------------|
| `index.ts` | [UI] [INTEGRATION] | contextBridge.exposeInMainWorld('electronAPI', ...). Typed API surface: auth, timer, projects, config, time, tasks, app, idle. Event listeners for timer updates, screenshots, config changes. |
