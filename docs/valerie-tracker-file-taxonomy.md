...
| `tracker-config.ts` | [AUTH] [INTEGRATION] | Reads config.json from ProgramData, safeStorage caching, pings server to validate API key, fetches + merges server config, offline fallback. Core auth module for production mode. **config.json only requires `apiKey` and `apiBaseUrl`** -- the server resolves `orgId` and `userId` from the API key via /api/tracker/ping and /api/tracker/config. All other settings are optional (server-provided). In production, va-platform provisions config.json automatically via AWS SSM RunCommand (`deployTrackerConfig()`). TrackerSettings includes autoStopIdleMin (default 15) for prolonged idle auto-stop (v0.2.7). |
...
| `auto-launch.ts` | | Windows Registry Run key for auto-start on login. Tries HKLM\...\Run first (machine-wide, persists in golden images), falls back to HKCU\...\Run if not elevated. Cleans up old HKCU key on successful HKLM write (v0.3.7). |
...
| File | Tags | Description |
|------|------|-------------|
| `installer.nsh` | [BUILD] | NSIS custom install/uninstall macros. On install: copies unpacked extension to C:\ProgramData\ValerieAgent\chrome-extension\, copies .crx and update.xml to C:\ProgramData\ValerieAgent\, cleans up old CRX registry keys (HKLM\SOFTWARE\Google\Chrome\Extensions\ for both old and new extension IDs), writes enterprise force-install policy via HKLM\SOFTWARE\Policies\Google\Chrome\ExtensionInstallForcelist (value: `{extensionId};file:///C:/ProgramData/ValerieAgent/update.xml`). Also writes machine-wide auto-launch Run key at HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run (v0.3.7). On uninstall: removes force-install policy key, removes old CRX registry keys, deletes extension files. Also removes HKLM and HKCU auto-launch Run keys (v0.3.7). Force-install replaces CRX sideload approach after Chrome 145 blocked sideloaded extensions (v0.3.5). |
...
| `pack-extension.js` | [BUILD] | Packs chrome-extension/ folder into CRX3 binary (valerie-url-bridge.crx) via `chrome.exe --pack-extension`. Requires Chrome installed on build machine. Uses existing extension.pem for consistent extension ID. No npm dependencies. Outputs CRX path, extension ID, and file size. Runs automatically before electron-builder in build:agent and publish:agent scripts (v0.3.2). Rewritten from CRX2 Node.js implementation to CRX3 Chrome CLI after Chrome 145 rejected CRX2 with CRX_HEADER_INVALID (v0.3.3). |
| `update.xml` | [BUILD] | Chrome enterprise update manifest for force-install policy. XML file referencing the local CRX path (`file:///C:/ProgramData/ValerieAgent/valerie-url-bridge.crx`) with extension ID and version. Installed to C:\ProgramData\ValerieAgent\ by NSIS installer. Referenced by ExtensionInstallForcelist registry policy (v0.3.5). |
...
| `INTEGRATION-GUIDE.md` | [INTEGRATION] | Moved to docs/INTEGRATION-GUIDE.md. Auth swap design, integration boundary, 18-task checklist (all DONE). Includes v0.3.5 test results, SSM provisioning documentation, and current va-platform integration status. |
...
| `agent/electron-builder.yml` | [INTEGRATION] | NSIS installer config. perMachine: true, asarUnpack for native modules, GitHub Releases publish. extraResources includes chrome-extension/ (unpacked source), valerie-url-bridge.crx (packed extension), and update.xml (force-install manifest) for NSIS installer bundling. nsis.include points to build/installer.nsh for extension force-install policy setup (v0.3.0, updated v0.3.5). |
...
| `agent/package.json` | | Agent workspace: scripts (dev, build, publish), dependencies, version 0.3.7. |
